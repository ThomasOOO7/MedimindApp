-- Update the guardian notification trigger to include actual medication time
...
BEGIN
  ...
  IF NEW.actual_time IS NOT NULL THEN
    v_actual_time_formatted := TO_CHAR(NEW.actual_time, 'HH12:MI AM');
  ELSE
    v_actual_time_formatted := TO_CHAR(NEW.scheduled_time, 'HH12:MI AM');
  END IF;
